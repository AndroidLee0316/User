...
/**
 * Meaning:
 * Created by 吕英玮 on 2018/5/8.
 * Email: lvyingwei134@pingan.com.cn
 */

public class IdCardUtils {
    public static final String[] ValCodeArr =
   ...
   /* public static String validate_effective(String idcardNumber,boolean isreturn_AutoCard){
    ...
    public static boolean validate_effective(String idcardNumber) {
        ...
        //校验身份证最后一位 身份证校验码
        if (!calculateVerifyCode(Ai).equalsIgnoreCase(
                String.valueOf(Ai.charAt(NEW_CARD_NUMBER_LENGTH - 1)))) {
            return false;
        }
        return true;
    }
     ...
     * 根据身份证判断性别
     */
    public static String getSex(String idcardnumber) {
        String sex;
        String id = idcardnumber.substring(16, 17);
    ...
    public static String getBirth(String idcardnumber) {
        return idcardnumber.substring(6, 10)
                + "-"
                + idcardnumber.substring(10, 12)
                + "-"
                + idcardnumber.substring(12, 14).toString();
    }
}
